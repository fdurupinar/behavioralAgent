...
(kbop:method
 ...
 :pre ((some (is-ask-what ONT::STATUS ?goal ?what)
       ...
       (some (dbname ?affected ?target-id))
       (get-term-xml ?affected ?affected-xml)

       (agent ?st ?agent)
       (some (dbname ?agent ?source-id))
...
(kbop:method
 :matches ((update-causality-model ?affected-xml ?goal ?reply-id))

 :on-ready ((:gentemp ?query-id "causality-update-model-query-")
            (:store (query ?query-id (update-causality-model
                                                              :source ?affected-xml)))
         ;;   (:subgoal (ask-bioagents ?query-id ?goal))
            ))
...
(kbop:method
 ...
 :on-ready ((:gentemp ?query-id "causality-query-")
            ...
            ;;Create a new query
            (:subgoal (ask-correlation ?affected-xml ?goal ?reply-id))
            ;;(:say "I asked correlation")
            ;;(:subgoal (update-causality-model ?affected-xml ?goal ?reply-id))
            ;;(:say "I asked causality model")
            ))
